...
//  private val DefaultTCompactProtocolSerializer = new TSerializer(new TCompactProtocol.Factory())
  ...
  def extractIterElements(elm: Any, isOptional: Boolean, fieldInfo: ThriftStructFieldInfo, thriftSerializer: TSerializer):Option[Iterable[Any]] = {
    if (isOptional)
      elm.asInstanceOf[Option[Iterable[Any]]].map(_.map(e => convertJavaElmToRowElm(e, fieldInfo, codec)))
    else
      Option(elm.asInstanceOf[Iterable[Any]].map(e => convertJavaElmToRowElm(e, fieldInfo, thriftSerializer)))
  }
    ...
    fieldInfo.tfield.`type` match {
      ...
      case TType.STRUCT =>
        if(fieldInfo.manifest.runtimeClass.getName == codec.metaData.structClassName){
          ...
          if(fieldInfo.isOptional) elm.asInstanceOf[Option[ThriftStruct with Product]].map(value => convert(value, codec)).orNull
